...
export function Studio() {
  ...
  return (
    ...
    >
      {/* Ambient orb */}
      <div
        className="animate-orb pointer-events-none absolute -top-32 -left-32 w-[600px] h-[600px] rounded-full opacity-[0.06]"
        style={{
      ...
      <div className="relative max-w-7xl mx-auto px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-20">
          <div>
            ...
            <h2
              className={`text-5xl sm:text-6xl md:text-7xl text-foreground max-w-2xl transition-all duration-700 delay-100 ${
                inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
              }`}
              style={{
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {PROJECTS.map((project, i) => (
            <article
              key={project.title}
              className={`group relative p-8 cursor-pointer border border-white/[0.06] -mt-px -ml-px
            ...
            >
              ...
              <span
                className="absolute top-4 right-5 text-7xl text-white/[0.04] group-hover:text-white/[0.08] transition-colors select-none pointer-events-none"
              ...
              {/* Thin accent line — grows on hover */}
              <div className="w-8 h-px bg-white/20 group-hover:w-16 group-hover:bg-white/40 transition-all duration-500 mb-5" />

              <h3
                className="text-2xl text-foreground mb-3 group-hover:text-white transition-colors"
              ...
              <span className="inline-flex items-center gap-2 mt-8 text-[10px] text-white/25 group-hover:text-white/60 transition-all duration-300 tracking-[0.2em] uppercase">
                View project
