...
export function About() {
  ...
  return (
    ...
    >
      ...
      <div
        className="animate-orb-2 pointer-events-none absolute -top-20 -right-40 w-[700px] h-[700px] rounded-full opacity-[0.05]"
        style={{
      ...
      <div className="relative max-w-7xl mx-auto px-8">
        {/* Label */}
        <p
          className={`text-xs tracking-[0.2em] uppercase text-muted-foreground mb-16 transition-all duration-700 ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start mb-28">
          ...
          >
            ...
            <div
              className="text-8xl text-white/[0.06] leading-none mb-2 select-none"
        ...
        {/* Stats — large and atmospheric */}
        <div
          className={`grid grid-cols-1 sm:grid-cols-3 gap-px border border-white/[0.06] transition-all duration-700 delay-300 ${
            inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
          }`}
        >
          {STATS.map((stat, i) => (
            <div
              key={stat.label}
              className="group relative p-10 border border-white/[0.06] -mt-px -ml-px hover:bg-white/[0.03] transition-colors"
              style={{ transitionDelay: `${400 + i * 80}ms` }}
            >
              <div
                className="text-7xl sm:text-8xl text-foreground mb-3 leading-none"
                style={{ fontFamily: "'Instrument Serif', serif" }}
              >
                {stat.value}
                <span className="text-4xl text-muted-foreground/50">{stat.suffix}</span>
              </div>
        ...
        {/* Closing line */}
        <p
          className={`text-center mt-24 text-muted-foreground/50 text-xl transition-all duration-700 delay-500 ${
            inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
          }`}
